...
// @Query("SELECT s.id FROM Stock AS s left join RentalManage AS r on s.id = r.stock.id join BookMst AS b on s.bookMst.id = b.id WHERE b.id = ?1 AND s.status = 0 AND (r.status is null or r.expectedRentalOn > ?2 OR ?2 > r.expectedReturnOn)")
...
"GROUP BY s.id",
nativeQuery = true)
    List<Object[]> calender(Long bookId, Date n);
}
